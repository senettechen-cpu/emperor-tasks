...
const MainDashboard = ({ currentUser, onLogout }: { currentUser: any, onLogout: () => void }) => {
  ...
  return (
    <div className={`h-screen bg-black relative flex flex-col items-center overflow-hidden transition-all duration-300 ${corruption > 950 ? 'glitch-container' : ''}`}>
      <div className="scanline" />
      ...
      <header className="w-full flex flex-col md:flex-row justify-between items-center p-4 md:p-6 border-b border-imperial-gold/20 z-10 bg-black/80 backdrop-blur-sm gap-4 md:gap-0">
        ...
        >
          <h4 className={`!m-0 !tracking-[0.5em] transition-colors duration-300 text-sm md:text-xl font-bold ${corruption > 950 ? '!text-red-500 animate-pulse' : (corruption > 500 ? '!text-green-500' : '!text-mechanicus-red')}`}>
            <span>{corruption > 950 ? '!! 亞空間裂隙 !!' : (corruption > 500 ? '亞空間能量：高' : '偵測亞空間能量')} <span className="font-mono opacity-60 ml-2 text-xs">({Math.floor(corruption)}/1000)</span></span>
          </h4>
          <div className="w-full md:w-64 h-2 bg-zinc-900 border border-mechanicus-red/30 mt-2 relative overflow-hidden">
            <div
              className={`h-full transition-all duration-1000 ${corruption > 800 ? 'bg-red-600' : 'bg-mechanicus-red'}`}
              style={{ width: `${corruption / 10}%` }}
            />
          </div>
        </div>
